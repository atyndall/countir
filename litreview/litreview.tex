...
% \roll \cite{rfc6550} is a routing protocol designed to replace those functions of TCP

% \lowpan \cite{shelby20116lowpan} is a
...
\subsection{Devices}
In addition to the protocol stack used, the format of the embedded systems must be decided on. 
% ``Smart Gateway'' \cite{guinard2011internet}
...
\node (rpi) [box, below=of http] {Smart Gateway};
\node (coap) [dashbox, right=of rpi] {\small \acs{coap}};
\node (embed) [box, right=of coap] {Sensor};

\draw [line] (interwebs) -- (http);
\draw [line] (http) -- (rpi);
\draw [line] (rpi) -- (coap);
\draw [line] (coap) -- (embed);
\end{tikzpicture}
